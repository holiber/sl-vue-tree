...
interface ISvtTreeData {
  svtTree?: SvtTree<any>
}

interface ISvtTreeMethods {

}

interface ISvtTreeComputed {
  tree: SvtTree<any>;
  level: number;
  parent: TSvtTreeComponent;
  root: TSvtTreeComponent;
}

interface ISvtTreeProps {
  value: ISvtNodeModel<any>[];
  path?: number[];
}
...
export default Vue.extend<ISvtTreeData, ISvtTreeMethods, ISvtTreeComputed, ISvtTreeProps>({
  data() {
    ...
    return {
      svtTree: path.length === 0 ? new SvtTree<any>(this.value) : null
  ...
  computed: {
    ...
    parent(): TSvtTreeComponent {
      if (!this.level) return this;
      return this.$parent as TSvtTreeComponent;
    },
...
//
//   created() {
//     this.tree = new SvtTree<any>(this.value as ISvtNodeModel[]);
//   }
//
//   mounted() {
//     if (this.isRoot) {
